...
# adapted from https://github.com/philss/rustler-precompiled-action/blob/main/build.sh

# Usage: ./build.sh "$(pwd)" "$(pwd)" "x86_64-unknown-linux-gnu" 2.17 true "--features a,b --no-default-features" false
#
...
tool=$(build_tool "$use_cross" "$use_zig")

# Only check cross config when using cross (not zigbuild)
if [ "$tool" = "cross build" ]; then
